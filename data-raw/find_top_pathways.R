...
reactome_levels <- full_join(reactome_names, reactome_top, multiple = 'all') %>%
...
# some of these will have character(0) as top pathway because some belong to multiple top pathways.
# Some other pathways also were originally annotated to multiple pathways.
# Annotate these 39 manually to a single top pathway
...
# also, 6 pathways from SIGORA are outdated, but need to manually add them in for sigora to map properly
sigora_pathways <- reaH$pathwaydescriptions
...
# Check that there are no more duplicate pathway ids that belong to multiple top pathways?
...
reactome_all_grouped <- reactome_all_annotated %>%
  mutate(grouped_pathway = case_when(
    top_pathway_name %in% c('Autophagy', 'ECM organization', 'Organelle biogenesis',
                            'Programmed Cell Death', 'Protein localization',
                            'Transport small molecules', 'Vesicle-mediated transport') ~ 'Cell Process',
    top_pathway_name %in% c('Cell Cycle', 'Chromatin organization', 'DNA Repair', 'DNA Replication') ~ 'Cell Replication',
    top_pathway_name %in% c('Gene expression') ~ 'Gene Expression',
    top_pathway_name %in% c('Hemostasis', 'Immune System') ~ 'Immune/Hemostasis',
    top_pathway_name %in% c('Metabolism', 'Metabolism of proteins', 'Metabolism of RNA', 'Drug ADME') ~ 'Metabolism',
    top_pathway_name %in% c('Cell responses to stimuli', 'Cell-Cell communication', 'Signal Transduction') ~ 'Signaling',
    top_pathway_name %in% c('Circadian Clock', 'Developmental Biology', 'Digestion and absorption', 'Muscle contraction',
                            'Neuronal System', 'Reproduction', 'Sensory Perception') ~ 'Tissue Function',
    top_pathway_name %in% c('Disease') ~ 'Disease'
  ))

# # the "Disease" top pathway varies, annotate them to the closest non-disease pathway's top pathway
...
# #reactome_genes <- read.csv('https://reactome.org/download/current/Ensembl2Reactome.txt', sep = '\t', header = FALSE)
# reactome_genes_data <- read.csv('https://reactome.org/download/current/NCBI2Reactome_All_Levels.txt', sep = '\t', header = FALSE)
#
# reactome_genes <- reactome_genes_data %>%
#   transmute(gene_id = V1,
#             pathway_id = V2,
#             pathway_name = V4) %>%
#   filter(grepl('HSA', pathway_id))
#
# # are all the disease pathways in this database?
# # five that are not present, deal with them manually afterwards
...
# annotated_pathways <- unique(reactome_all_grouped %>% filter(!is.na(grouped_pathway)) %>% .$pathway_id)
#
# distance_matrix_filtered <- distance_matrix[
#   !row.names(distance_matrix) %in% annotated_pathways, # rows are all the pathways that aren't already annotated
#   annotated_pathways[annotated_pathways %in% row.names(distance_matrix)] # columns are all the pathways that have annotation
#   ]
#
...
#     .$grouped_pathway
#   disease_grouped_pathway <- rbind(disease_grouped_pathway,
...
# )
#
# # check it over and manually change some that don't make sense




# For mSigDB Hallmark gene sets -----------------------------------------------
...
development <- c("MYOGENESIS", "ANGIOGENESIS", "SPERMATOGENESIS", "ADIPOGENESIS",
                 "EPITHELIAL MESENCHYMAL TRANSITION", "PANCREAS BETA CELLS")
dna_damage <- c("DNA REPAIR", "UV RESPONSE DN", "UV RESPONSE UP")
immune <- c("ALLOGRAFT REJECTION", "COMPLEMENT", "COAGULATION",
            "INFLAMMATORY RESPONSE", "INTERFERON ALPHA RESPONSE", "INTERFERON GAMMA RESPONSE")
metabolism <- c( "BILE ACID METABOLISM", "CHOLESTEROL HOMEOSTASIS", "GLYCOLYSIS", "XENOBIOTIC METABOLISM",
                 "FATTY ACID METABOLISM", "HEME METABOLISM", "OXIDATIVE PHOSPHORYLATION")
stress <- c("HYPOXIA", "APOPTOSIS",  "UNFOLDED PROTEIN RESPONSE",
            "PROTEIN SECRETION", "REACTIVE OXYGEN SPECIES PATHWAY")
proliferation <- c("E2F TARGETS", "G2M CHECKPOINT", "MITOTIC SPINDLE", "P53 PATHWAY",
                   "MYC TARGETS V1", "MYC TARGETS V2")
signaling <- c('ANDROGEN RESPONSE', "ESTROGEN RESPONSE EARLY", "ESTROGEN RESPONSE LATE",
               "KRAS SIGNALING DN", "KRAS SIGNALING UP", "MTORC1 SIGNALING", "NOTCH SIGNALING",
               "HEDGEHOG SIGNALING", "PI3K AKT MTOR SIGNALING", "TGF BETA SIGNALING",
               "IL6 JAK STAT3 SIGNALING", "IL2 STAT5 SIGNALING", "TNFA SIGNALING VIA NFKB", "WNT BETA CATENIN SIGNALING")
...
# 1. used all Reactome pathways instead of just SIGORA pathways (2,621 pathways over 1,298 originally)
# 2. Disease is now a grouped pathway instead of separating into other grouped pathways
