...
Gene expression studies such as microarrays and RNA-Seq often result in
...
as the object `exampleDESeqResultsHS`. This is a list of 2 data frames, generated
...
exNetworkPathways <- ppiEnrichNetwork(
    ...
    geneUniverse=rownames(exampleDESeqResultsHS[[1]])
)
```
...
Note the structure of `exampleDESeqResultsHS`, a named list of results from
DESeq2:
...
## Drop the "genes" column for better visibility
...
pathwayDistancesJaccard <- getPathwayDistances(pathwayData=sigoraDatabaseHS)
