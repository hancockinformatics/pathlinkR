...
as the object `deseqExampleList`. This is a list of 2 data frames, generated
...
significant by `enrichPathway()`). The function `plotFoldChange()` accomplishes
this by taking in an input list of `DESeq2::results()` data frames, just like
`enrichPathway()`, and creating a heatmap of fold changes for the constituent
genes.
...
deseqExampleListRenamed <- setNames(
    deseqExampleList,
    c("Pos", "Neg")
)
...
function  `enrichPathway()`.

The `enrichPathway()` function takes as input a list of data frames (each from
...
`analysis="sigora"` we also need to provide a special object, the Gene Pair
Signature Repository (`gpsRepo`) which contains the pathways and gene pairs to
be tested; **pathlinkR** comes with the object `reaH` from **Sigora** containing
the human Reactome pathways pre-computed and ready-to-use.
```{r sigora_enrichment}
...
exGeneUniverse <- mappingFile %>% 
  filter(ensemblGeneId %in% rownames(deseqExampleList[[1]])) %>% 
  pull(entrezGeneId)
...
plotPathways(
  enrichResults=enrichedResultsSigora, 
  columns=2
)
```
...
`enrichPathway()`). Size of nodes is correlated with statistical significance,
while edge thickness relates to the similarity of two connected pathways.
...
**Sigora** uses a Gene-Pair Signature (GPS) Repository that stores information on
