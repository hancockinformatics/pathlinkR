...
significant by `pathwayEnrichment()`). The function `plotFoldChange()` accomplishes
this by taking in an input list of `DESeq2::results()` data frames, just like
`pathwayEnrichment()`, and creating a heatmap of fold changes for the constituent
genes.
...
The `pathwayEnrichment()` function takes as input a list of data frames (each from
...
`pathwayEnrichment()`). Size of nodes is correlated with statistical significance,
